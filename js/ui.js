...
export function showView(viewId) {
    DOMElements.appSections.forEach(section => {
    ...
    if (viewId === 'presupuestos-view') {
        ...
        document.getElementById('acciones-finales-view').style.display = 'flex';
    } else {
...
export function renderPresupuestoFinal(presupuesto) {
    ...
    DOMElements.presupuestoMeta.innerHTML = `
        <div class="meta-line">
            <span><strong>Nº Presupuesto:</strong> ${DOMElements.presupuestoNumeroPresupuesto.value || 'N/A'}</span>
        </div>
        <div class="meta-line">
            <span><strong>Fecha:</strong> ${DOMElements.presupuestoFechaPresupuesto.value || 'N/A'}</span>
        </div>
    `;
}
...
export function renderPresupuestosList(presupuestos) {
    ...
    if (!presupuestos || presupuestos.length === 0) {
        DOMElements.listaHistorial.innerHTML = '<li class="collection-item">No hay presupuestos para mostrar.</li>';
        return;
    }
