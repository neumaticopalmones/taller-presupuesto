...
export function getCurrentPresupuesto() {
  return state.currentPresupuesto;
}
export function getCurrentEditPresupuestoId() {
  return state.currentEditPresupuestoId;
}
export function getInventario() {
  return state.inventario;
}
export function getCurrentEditInventarioItem() {
  return state.currentEditInventarioItem;
}
export function getHistorial() {
  return state.historial;
}
export function getPaginacion() {
...
export function removeTempNeumatico(id) {
  state.currentPresupuesto.tempNeumaticos = state.currentPresupuesto.tempNeumaticos.filter(
    (n) => n.id !== id
  );
}
...
export function removeTempTrabajo(id) {
  state.currentPresupuesto.tempOtrosTrabajos = state.currentPresupuesto.tempOtrosTrabajos.filter(
    (t) => t.id !== id
  );
}
...
export function updateTempNeumaticosParams(params = {}) {
  ...
  state.currentPresupuesto.tempNeumaticos = state.currentPresupuesto.tempNeumaticos.map((n) => {
    ...
    const cantidad =
      Number.isFinite(parseInt(cantidadRaw, 10)) && parseInt(cantidadRaw, 10) > 0
        ? parseInt(cantidadRaw, 10)
        : n.cantidad || 1;
    const ganancia = Number.isFinite(parseFloat(gananciaRaw))
      ? parseFloat(gananciaRaw)
      : n.ganancia || 0;
    const ecotasa = Number.isFinite(parseFloat(ecotasaRaw))
      ? parseFloat(ecotasaRaw)
      : n.ecotasa || 0;
...
export function addGroupToPresupuesto() {
  ...
  const neumaticosSnapshot = (neumaticos || [])
    .map((n) => ({ ...n }))
...
export function updateGroupsWithParams(params = {}) {
  ...
  state.currentPresupuesto.grupos.forEach((grupo) => {
    ...
    const cantidad =
      Number.isFinite(parseInt(params.cantidad, 10)) && parseInt(params.cantidad, 10) > 0
        ? parseInt(params.cantidad, 10)
        : grupo.cantidad;
    const ganancia = Number.isFinite(parseFloat(params.ganancia))
      ? parseFloat(params.ganancia)
      : grupo.ganancia;
    const ecotasa = Number.isFinite(parseFloat(params.ecotasa))
      ? parseFloat(params.ecotasa)
      : grupo.ecotasa;
    ...
    if (Array.isArray(grupo.neumaticos)) {
      grupo.neumaticos = grupo.neumaticos
        .map((n) => {
          const neto = Number.isFinite(parseFloat(n.neto)) ? parseFloat(n.neto) : 0;
          const baseSinGanancia = neto + ecotasa;
          const puSinGanancia = baseSinGanancia * (1 + iva / 100);
...
export function removeElementFromGroup(grupoId, elementoId, tipo) {
  ...
  if (grupo.neumaticos.length === 0 && grupo.otrosTrabajos.length === 0) {
    state.currentPresupuesto.grupos = state.currentPresupuesto.grupos.filter(
      (g) => g.id !== grupoId
    );
  } else {
    // Recalcular total del grupo
...
export function chooseMarcaAndRemoveSameMeasure(grupoId, neumaticoId) {
  ...
  state.currentPresupuesto.grupos = state.currentPresupuesto.grupos.filter((g) => {
    const medidaGrupo =
      g.medida || (g.neumaticos && g.neumaticos[0] && g.neumaticos[0].medida) || null;
